...
/*
		...
		Copyright (c) 1985,1987 William C. Colley, III
           these fixes copyright (c) Herb Johnson 2026
...
0.1++ Dec 2 2022 added LIST , PRINT pseudo op

0.1+  Mar 20 2023 symbol table file
0.1+4 Jan 2025  added Chapman IFDEF IFNDEF
      Aug 2026 fix to sclose() Jim Michener
...
extern int liston; /* HRJ list pseudo op */
...

{
	...
	{ UNARY  + UOP3  + OPR,				0,	"OFF"	}, /* LIST must have lexical values HRJ */
	{ UNARY  + UOP3  + OPR,				1,	"ON"	}, /* LIST must have lexical values HRJ */
...
static FILE *sym = NULL;  /* symbol table listing file routines */

void sopen(char *nam)
{
    if (sym) warning(TWOLST);
    else if (!(sym = fopen(nam,"w"))) fatal_error(SYMOPEN);
    return;
}
...
static void file_sym(SYMBOL *sp) /* call with  file_sym(sroot) */
{
	char lead;

    if (sp) {
	file_sym(sp -> left);
	if ((sp ->valu) >= 0xA000) lead = '0'; else lead = ' ';
	/* busy way to produce leading 0 if hex value is A000H-FFFFH */
	fprintf(sym,"%-10s  EQU  %c%04XH\n",sp -> sname,lead,sp -> valu);
	/* if (scol = ++scol % SYMCOLS) fprintf(sym,"    ");
	else {
	    fprintf(sym,"\n");
	}*/
	file_sym(sp -> right);
...
void sclose(void)
{
	if (!sym) return; /*Michener*/
	if (sroot) {
		file_sym(sroot);
    	if (sym) {
		fprintf(sym,"\0");
		if (ferror(sym) || fclose(sym) == EOF) fatal_error(DSKFULL);
    		}
	}
	return;
}
...
void lputs(void)
{
    ...
    if (list && liston) {  /* HRJ added LIST pseudo op */
	i = bytes;  o = obj;
	do {
