...
0.1+4	Jan 2025	added Chapman suppress() routine
...
/* Allow suppression of UNDEFINED LABEL error for one lex */
static int suppress_undefined = FALSE;
    ...
    if (isalph(c = popc())) {
		...
		if (!strcmp(token.sval,"$")) {
	    	...
	    	if (s = find_symbol(token.sval)) {
				token.valu = s -> valu;
				suppress_undefined = FALSE; /* HRJ clear suppression if true */
				if (pass == 2 && s -> attr & FORWD) forwd = TRUE;
	   		}
			else if (suppress_undefined) {
				/* Allow it through for one lex */
...
int popc(void)
{
	...
	if (c == EOF) c = '\n';
	if (c == '!' && !quote) { //HRJ permits !<sp> to combine lines of assembly
		eol = TRUE;  *lptr++ = '!'; // as in pop h! pop d
		*lptr++ = '\n'; *lptr = '\0'; // must be space after !
		return '\n';
	}
...
/***********************************************************
 * suppress() -- Suppress UNDEFINED LABEL errors (Chapman)
 */
void suppress() {
